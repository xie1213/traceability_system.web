<template>
    <div style="height: 100%; width: 100%">
        <div style="padding-left: 20px; padding-right: 20px">
            <el-text style="padding-right: 50px"
                >内存硬盘剩余容量
                <el-button style="background-color: #459cfb; color: white" disabled>{{ AvailableSpaceGB }}</el-button>
            </el-text>
            <el-text style="padding-right: 50px"
                >已使用容量 <el-button style="background-color: #459cfb; color: white" disabled>{{ FolderSize }}</el-button>
            </el-text>
            <el-text style="padding-right: 50px"
                >当前时间 <el-button style="background-color: #459cfb; color: white" disabled>{{ formattedTime }}</el-button>
            </el-text>
            <el-text style="padding-right: 50px"
                >通讯状态
                <btnIPCheck />
...
<script setup>
    ...
    const getDriveSpace = () => {
        apiClient
            .get("api/DiskSpace/GetDDriveSpace")
            .then((res) => {
