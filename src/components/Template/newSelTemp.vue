<template>
    ...
    <div class="table_calss" v-if="tableName != '出荷履历'">
        ...
        <el-select
            v-model="firstValue"
            @clear="clearFirstItem"
            :class="{ IsChecked: isSelectChecked }"
            clearable
            placeholder="Select"
            style="width: 150px">
            <el-option v-for="(item, index) in firstList" :key="index" :label="item" :value="item" @click="handleItemClick(index)" />
        </el-select>
        <!-- 全部选择 -->
        <!-- {{ firstValue }} -->
    ...
    <div class="limit_class" v-if="showDate != true">
        ...
        <el-input
            v-model="selRequestData.topLimit"
            :class="{ IsChecked: isSelectChecked }"
            style="width: 150px"
            @input="topLimitInput"
        ...
        <el-input
            v-model="selRequestData.lowerLimit"
            :class="{ IsChecked: isSelectChecked }"
            style="width: 150px"
            @input="lowerLimitInput"
            placeholder="Please input" />
    </div>
    <!-- 时间控件 -->
